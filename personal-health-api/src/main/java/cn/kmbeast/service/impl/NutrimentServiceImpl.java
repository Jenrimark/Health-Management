...
public class NutrimentServiceImpl implements NutrimentService {
     ...
     */
    @Override
    public Result<String> deleteUserNutriments(List<Integer> ids) {
        if (CollectionUtils.isEmpty(ids)) {
            return ApiResult.error("请选择要删除的营养素");
        }

        // 获取当前用户ID
        Integer userId = LocalThreadHolder.getUserId();

        // 执行删除操作，确保只能删除当前用户的私人营养素
        int deletedCount = nutrimentMapper.deleteUserNutriments(ids, userId);

        if (deletedCount == 0) {
            return ApiResult.error("删除失败：只能删除自己的私人营养素");
        }

        // 检查是否所有ID都成功删除
        if (deletedCount < ids.size()) {
            return ApiResult.success("成功删除部分记录（只能删除自己的私人营养素）");
        }
